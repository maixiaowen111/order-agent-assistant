...
## 10. 批准链路写死了「取消订单」，第二个写操作会被带偏

**现象**：加第二个写操作工具「改地址」后，用户说「帮我把订单 X 的收货地址改成 Y」，模型调 `update_order_address` → 被闸门拦 → 用户点「批准执行」→ agent 却跑去调 `cancel_order` **把订单取消了**。

**根因**：批准链路三处文案都写死了「取消订单」：
- `WritePermissionGate.reason()`：返回「写操作被拦截：**取消订单** X 需要人工确认」
- `AgentLoop.markApproved()`：批准后注入「请立即调用 **cancel_order** 工具执行」
- 前端 `approve()` 的 follow-up：补发「我已批准，请继续执行**取消订单** X」

批准是**通用动作**（闸门只管「这个会话有没有被批准」，不关心具体是哪个写操作），文案却绑死了第一个写操作。模型看到「请执行取消订单」，自然照做——改地址请求被批准后反而取消了订单。

**修复**：三处全部泛化——
- `reason()`：说「写操作被拦截：需要人工确认后才能执行（订单 X）」，带 orderNo 保留上下文
- `markApproved()`：说「我已批准你刚才要执行的写操作，请立即调用**对应的工具**完成它」
- 前端 follow-up：说「请继续执行**你刚才要执行的写操作**」

模型知道自己刚才在做什么，让它按上下文决定调哪个工具。

**面试怎么讲**：这个坑的价值是「**通用机制不能长成单点**」。闸门拦截/批准是通用能力，但第一版把「取消订单」这个具体业务写进了通用文案——加第二个写操作时才暴露出来。教训：**机制层（闸门）和业务层（工具）要分离，通用文案别点名具体工具**。面试官问「你闸门怎么设计的」时，能主动讲这个演进（从 cancel 专用改成通用，为什么改）比只讲「写操作要批准」深一层。
