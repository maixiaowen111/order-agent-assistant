...
一个 **AI Agent 管理订单** 的可运行项目：用户用自然语言查单、取消订单；取消已支付订单会自动触发退款，并在通知中心生成一条真实的退款通知。
...
- **端到端验证通过**：真实 DeepSeek 模型跑通「查单 → 取消被拦截 → 人工批准 → 取消成功 → 退款事件 → 通知落库」。
...
订单管理就「查单 / 取消 / 退款通知」三件事，一个循环足够。多 Agent 解决的是"复杂组织的协调"问题——这里没有，硬上只会增加延迟和 token 成本。真正的关键是**权限闸门**：把"人工确认"这个动作放进决策流，模型负责决策，闸门负责边界。
...
```bash
cd order-agent-assistant && mvn test    # 17 个用例：AgentLoop / 闸门 / 会话存储 / 消息往返
