...
| 并发控制 | MySQL 原子 UPDATE + 影响行数闸门（Redisson 仅备用） |
...
- **原子扣减防超卖**：`UPDATE t_product SET stock=stock-? WHERE id=? AND stock>=?`，影响行数=0 即并发超卖被拦下——比"查-改-写 + 锁"更简单，天然并发安全
- **缓存两防（已实现）**：空值防穿透、随机 TTL 防雪崩；**热点 Key 互斥重建（防击穿）尚未实现**——README 如实，留作后续
- **状态机重试**：WAIT→PROCESSING→SUCCESS/FAIL，PROCESSING 防并发重复处理
- **支付/退款如实描述**：支付是**状态模拟**（接口直接置 PAID，不接真实支付通道）；退款是**事件 + 通知**（取消已支付订单 → 插 REFUND 事件 → 调度器处理 → 通知中心生成退款通知）
