 ...
 *
 * 为什么：本地开发图省事，配置里给了开发默认值（JWT_SECRET 等），但生产环境绝不能带着公开的
 * 默认密钥上线——密钥泄露等于用户数据裸奔。这里在非 dev 环境把"缺失 / 用默认值"直接变成启动失败，
 * 让问题卡在部署环节，而不是靠人记着去改配置。
 *
 * 判定：spring.profiles.active == dev 放行（本地开发不设环境变量也能跑通）；其余环境一律校验。
 * 和 agent 侧的 RequiredEnvValidator 是同一套约定（各自校验各自依赖的密钥）。
 */
@Component
public class RequiredEnvValidator implements ApplicationRunner {

    /**
     * 生产必须设置的环境变量。value = 开发默认值（生产禁止使用；null 表示配置里根本没有默认值、环境必须设）。
     * 用 LinkedHashMap 保序：报错时优先提示第一个缺失项，一次只暴露一个问题，好排查。
     */
    ...
    static {
        ...
        REQUIRED_IN_NON_DEV.put("DB_PASSWORD", null);
    }
    ...
    public void run(ApplicationArguments args) {
        ...
        for (Map.Entry<String, String> entry : REQUIRED_IN_NON_DEV.entrySet()) {
            ...
            String devDefault = entry.getValue();
            String value = env.apply(name);
            if (value == null || value.isBlank()
                    || (devDefault != null && devDefault.equals(value))) {
                throw new IllegalStateException(
                        "生产环境必须设置环境变量 " + name
                                + (devDefault != null
                                        ? "（且不能使用开发默认值 " + devDefault + "）"
                                        : "（配置里没有默认值，缺失无法启动）")
                                + "。请检查部署环境。");
            }
        }
